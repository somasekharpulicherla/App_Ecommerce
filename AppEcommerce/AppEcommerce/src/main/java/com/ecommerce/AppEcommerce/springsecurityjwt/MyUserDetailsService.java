...
public class MyUserDetailsService implements UserDetailsService {
    ...
    public UserDetails loadUserByUsername(String username) throws UsernameNotFoundException {
    	...
    	if(!user.isPresent())
				throw new UsernameNotFoundException("User doesn't exist with Username or Email.");
        
            return User.builder().username(username).password(user.get().getPasscode()).build();
